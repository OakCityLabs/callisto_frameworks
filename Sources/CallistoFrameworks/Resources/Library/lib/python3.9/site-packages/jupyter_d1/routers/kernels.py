...
async def list_kernel_specs():
    ...
    for key in kspecs.keys():
        ...
        if (
            callisto_env in ("staging", "prod")
            and (
                tmp_spec["spec"]["metadata"].get("conda_env_name", None)
                == "jupyter_d1" or tmp_spec["resource_dir"] == 
                "/home/ubuntu/anaconda3/envs/jupyter_d1/share/jupyter/"
                "kernels/python3"
            )
        ):
            continue
    ...
    tmp_specs.sort(key=itemgetter("kernel_name"))
