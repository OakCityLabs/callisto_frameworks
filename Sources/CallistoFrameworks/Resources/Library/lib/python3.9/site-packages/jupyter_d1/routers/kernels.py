...
router = APIRouter()
kspec = jupyter_client.kernelspec.KernelSpecManager()
...
@router.get("/shutdown/{uuid}", dependencies=[Depends(write_access)])
async def shutdown_kernel(uuid: UUID, status_code=status.HTTP_204_NO_CONTENT):
    "Shutdown the kernel identified by the uuid"
    await kmanager.shutdown_kernel(uuid)


@router.get("/restart/{uuid}", dependencies=[Depends(write_access)])
async def restart_kernel(uuid: UUID, status_code=status.HTTP_204_NO_CONTENT):
    "Restart the kernel identified by the uuid"
    await kmanager.restart_kernel(uuid)


@router.get("/interrupt/{uuid}", dependencies=[Depends(write_access)])
async def interrupt_kernel(uuid: UUID, status_code=status.HTTP_204_NO_CONTENT):
