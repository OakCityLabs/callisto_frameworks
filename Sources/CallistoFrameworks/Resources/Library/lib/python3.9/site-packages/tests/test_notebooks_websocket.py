...
async def upload_notebook(
    ...
    response = await client.get(
        f"/notebooks/open/{path}", headers=token_headers
    )
    assert response.status_code == 201
...
class TestNotebookWebSocket:
    ...
    async def test_kernel_interrupt(
        ...
        async with client.websocket_connect(
            ...
            response = await client.get(
                f"/kernels/interrupt/{uuid}", headers=superuser_token_headers
            )
            assert response.status_code == 200
...
class TestChangeWorkingDirectory:
    ...
    async def do_test_change_working_directory_in_code(
        ...
        async with client.websocket_connect(
            ...
            data = await wait_for_event(websocket, "metadata")
