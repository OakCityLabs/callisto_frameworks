...
def upload_notebook(
    ...
    response = client.get(f"/notebooks/open/{path}", headers=token_headers)
    print(response.json())
    assert response.status_code == 201
...
class TestNotebook:
    ...
    def test_upload_to_directory(
        ...
        with open(f"tests/notebooks/simple.ipynb", "r") as f:
            nb_raw = f.read()
        response = client.post(
            "/notebooks/upload",
    ...
    def test_upload_to_directory_fail(
        ...
        assert (
            ...
            == "Directory does not exist /tmp/jupyter_d1_test/aproject"
        )

    def test_working_directory(
        self, client: TestClient, superuser_token_headers: Dict[str, str]
    ):
        nb_filename = f"tests/notebooks/simple.ipynb"
        nb_json = open(nb_filename).read()
        response = client.post(
            "/notebooks/upload",
        ...
        response = client.get(
            "/notebooks/open/name.ipynb", headers=superuser_token_headers
        )
        assert response.status_code == 201
        resp_json = response.json()
        assert (
    ...
    def test_upload_and_open_in_memory_working_dir_specified(
        ...
        assert (
            ...
            == f"{os.getcwd()}/tests/notebooks"
        )
        uuid = resp_json["notebook"]["metadata"]["jupyter_d1"]["uuid"]

        assert not os.path.exists(f"{settings.ROOT_DIR}/name.ipynb")
        # This should be impossible, but might as well check it
        assert not os.path.exists(f"{os.getcwd()}/tests/notebooks/name.ipynb")
    ...
    def test_upload_and_open_working_dir_defaults_to_root_dir(
        ...
        """
        try:
            os.remove(f"{os.getcwd()}/tests/notebooks/name.ipynb")
        except Exception:
            pass
        nb_filename = f"tests/notebooks/simple.ipynb"
        nb_json = open(nb_filename).read()
        response = client.post(
            ...
            params={
                "filename": "name.ipynb",
                "directory": f"{os.getcwd()}/tests/notebooks",
            },
            data=nb_json,
            headers=superuser_token_headers,
        )
        ...
        assert os.path.exists(f"{os.getcwd()}/tests/notebooks/name.ipynb")
