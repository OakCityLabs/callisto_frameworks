...
def upload_notebook(
    ...
    response = client.get(f"/notebooks/open?filepath={path}", headers=token_headers)
    assert response.status_code == 201
...
class TestNotebook:
    ...
    def test_round_trip(
        ...
        response = client.get(
            f"/notebooks/open?filepath={path}", headers=superuser_token_headers
        )
        assert response.status_code == 201
        notebook = response.json()["notebook"]
    ...
    def test_working_directory(
        ...
        response = client.get(
            "/notebooks/open?filepath=name.ipynb", headers=superuser_token_headers
        )
        assert response.status_code == 201
        resp_json = response.json()
        assert (
